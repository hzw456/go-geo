...
func TestS2GetCellID(t *testing.T) {
	...
	coord := geo.S2GetCellID(geo.Point{113.444956239154266, 23.167870240520788}, 13, geo.SRID_WGS84_GPS)
	t.Log(coord)
}
...
func TestS2RegionCoverer(t *testing.T) {
	geom, _ := wkt.Decode("POLYGON((113.444875331122134 23.167959059560499,113.444956239154266 23.167870240520788,113.444899423736146 23.167824392635907,113.444820493455495 23.167918965135584,113.444875331122134 23.167959059560499))")
	poly := geom.(geo.Polygon)
	t.Log(wkt.Encode(poly))
	cellID := geo.S2RegionCoverer(poly, 22, geo.SRID_WGS84_GPS)
	t.Log(cellID)
}
